...
template <typename Element>
void TestEmbedding::TestRoutine(const double tol_forward, const double tol_backward) {
...
//  const dllm::TensorIndexType d = 8;
  ...
  auto wte = torch::randn({vocab, d}, torch::TensorOptions().dtype(dtype).device(device));
  auto wpe = torch::randn({MaxT, d}, torch::TensorOptions().dtype(dtype).device(device));
  auto input = torch::randint(0, 3, {B, T}, torch::TensorOptions().dtype(torch::kInt).device(device));
  auto input_wpe = torch::range(0, T-1, torch::TensorOptions().dtype(torch::kInt).device(device)).repeat({B, 1});



  auto input1 = input.detach().clone();
  auto wte1 = wte.to(TypeToTorch<Element>::type).detach().clone().set_requires_grad(true);
  auto wpe1 = wpe.to(TypeToTorch<Element>::type).detach().clone().set_requires_grad(true);
  ...
  CHECK_CUDART(cudaMalloc(&DeviceOutput, sizeof(Element) * cute::size(layoutOutput)));

  CHECK_CUDART(cudaMemcpy(DeviceInput, input2.data_ptr<int>(),
                          sizeof(int) * cute::size(layoutInput),
                          cudaMemcpyHostToDevice));
  CHECK_CUDART(cudaMemcpy(DeviceWte, wte2.data_ptr<typename TypeToTorch<Element>::Type>(),
                          sizeof(Element) * cute::size(layoutWte),
                          cudaMemcpyHostToDevice));
  CHECK_CUDART(cudaMemcpy(DeviceWpe, wpe2.data_ptr<typename TypeToTorch<Element>::Type>(),
                          sizeof(Element) * cute::size(layoutWpe),
                          cudaMemcpyHostToDevice));
  ...
  auto task_forward = dllm::compute::embedding::forward(tensorOutput, tensorInput, tensorWte, tensorWpe);
  task_forward(&context);

  auto output2 = torch::empty_like(output1, torch::TensorOptions().dtype(dtype));

  CHECK_CUDART(cudaMemcpy(output2.template data_ptr<typename TypeToTorch<Element>::Type>(), DeviceOutput,
                          sizeof(Element) * cute::size(layoutOutput),
                          cudaMemcpyDeviceToHost));

  CHECK_CUDART(cudaDeviceSynchronize());

  auto isApprox_output = output1.allclose(output2.to(TypeToTorch<Element>::type), tol_forward);
...
// backward check
  ...
  // 计算梯度
  auto grads = torch::autograd::grad({output1}, {wte1, wpe1}, {grad_output}, /*retain_graph=*/false, /*create_graph=*/false, /*allow_unused=*/true);
  ...
  auto layoutGradOutput = cute::make_layout(shapeGradOutput, cute::GenRowMajor{});
  ...
  CHECK_CUDART(cudaMalloc(&Device_grad_output, sizeof(Element) * cute::size(layoutGradOutput)));
  CHECK_CUDART(cudaMalloc(&Device_grad_wte, sizeof(Element) * cute::size(layoutGradWte)));
  CHECK_CUDART(cudaMalloc(&Device_grad_wpe, sizeof(Element) * cute::size(shapeGradWpe)));
  CHECK_CUDART(cudaMemcpy(Device_grad_output, grad_output.to(dtype).template data_ptr<typename TypeToTorch<Element>::Type>(),
                          sizeof(Element) * cute::size(layoutGradOutput),
                          cudaMemcpyHostToDevice));
  CHECK_CUDART(cudaMemset(Device_grad_wte, 0, sizeof(Element) * cute::size(layoutGradWte)));
  CHECK_CUDART(cudaMemset(Device_grad_wpe, 0, sizeof(Element) * cute::size(layoutGradWpe)));
  auto tensorGradOutput = std::make_shared<dllm::Tensor3D>(
      Device_grad_output, layoutOutput, dllm::toDtype<Element>(), dllm::CUDA);
  auto tensorGradWte = std::make_shared<dllm::Tensor2D>(Device_grad_wte, layoutGradWte, dllm::toDtype<Element>(), dllm::CUDA);
  auto tensorGradWpe = std::make_shared<dllm::Tensor2D>(Device_grad_wpe, layoutGradWpe, dllm::toDtype<Element>(), dllm::CUDA);

  CHECK_CUDART(cudaDeviceSynchronize());



  auto task_backward = dllm::compute::embedding::backward(tensorGradOutput, tensorInput, tensorGradWte, tensorGradWpe);
  task_backward(&context);

  auto grad_wte2 = torch::empty_like(grad_wte1, torch::TensorOptions().dtype(dtype));
  auto grad_wpe2 = torch::empty_like(grad_wpe1, torch::TensorOptions().dtype(dtype));

  CHECK_CUDART(cudaMemcpy(grad_wte2.template data_ptr<typename TypeToTorch<Element>::Type>(), Device_grad_wte,
                          sizeof(Element) * cute::size(layoutGradWte),
                          cudaMemcpyDeviceToHost));
  CHECK_CUDART(cudaMemcpy(grad_wpe2.template data_ptr<typename TypeToTorch<Element>::Type>(), Device_grad_wpe,
  ...
  auto isApprox_grad_wte = grad_wte1.allclose(grad_wte2.to(TypeToTorch<Element>::type), tol_backward);
  auto isApprox_grad_wpe = grad_wpe1.allclose(grad_wpe2.to(TypeToTorch<Element>::type), tol_backward);
...
TEST_F(TestEmbedding, TestFloat) {
  TestRoutine<float>(1e-5, 1e-2);
}

TEST_F(TestEmbedding, TestHalf) {
  TestRoutine<half>(1e-5, 1e-2);
}