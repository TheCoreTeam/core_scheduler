...
class CuDnnAttentionTestFixture : public ::testing::Test {
 protected:
...
void TestT(cs::Scheduler& scheduler) {
  ...
  auto state = cs::compute::ScaledDotProductCuDnn::init(
      scheduler,
      cs::compute::ScaledDotProductCuDnn::Options{}.is_causal(true).scale(
          scale));
  auto qview =
  ...
  vview.wait();
  auto output = cs::compute::ScaledDotProductCuDnn::forward(
      scheduler, state, qview, kview, vview);
  output.wait();
  auto dout = cs::compute::Utils::rand_like(scheduler, output);
  output.wait();
  auto [dq, dk, dv] =
      cs::compute::ScaledDotProductCuDnn::backward(scheduler, state, dout);
  dq.wait();
...
TEST_F(CuDnnAttentionTestFixture, TestFP16) { TestT<half>(scheduler); }
TEST_F(CuDnnAttentionTestFixture, TestBF16) { TestT<nv_bfloat16>(scheduler); }
