...
from models.gpt2.data_parallel.gpt2 import GPTConfig, GPT2Model
from operators.communication import Dist
from operators.optim import dist_sgd as sgd
...
if __name__=="__main__":
    ...
    data_sampler = dist.get_sampler(train_dataset, num_replicas=dist.get_world_size(), rank=dist.get_rank())
    train_loader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=collate_fn, sampler=data_sampler)

    # Create GPT model
    model = GPT2Model(config, dist)
    
    # Create the optimizer
    optimizer = sgd.Zero1SGD(model.get_parameters(), lr=lr, momentum=momentum, dist=dist)
    
    # Training loop
    for epoch in range(epochs):
        if rank == 0:
            print(f"Epoch {epoch}")
        with MemoryTest(enable=True, tqdm=False, master_process=rank==0) as mem_test:
            with TimeTest(enable=True, tqdm=False, master_process=rank==0) as time_test:
                global_loss_value = 0
