...
namespace {
template <typename T>
__global__ void GeLU_forward(T* __restrict__ output, const T* __restrict__ input,
                     std::size_t n) {
...
__global__ void GeLU_backward(floatX* dinp, const floatX* inp, const floatX* dout) {
...
void forwardKernel(cudaStream_t cudaStream, Tensor2D& output,
  ...
  auto f = [&](auto dummy) {
    ...
    GeLU_forward<<<grid, block, 0, cudaStream>>>(static_cast<T*>(output.data()),
...
void backwardKernel(cudaStream_t cudaStream, Tensor2D& dinput,
  ...
  auto f = [&](auto dummy) {
    ...
    GeLU_backward<<<grid, block, 0, cudaStream>>>(static_cast<T*>(dinput.data()),
                                                  static_cast<const T*>(input.data()),
