...
//// use fp16 (note: this may require gradient scaler, currently not
/// implemented!)
...
// #if defined(ENABLE_BF16) && (__CUDACC_VER_MAJOR__ < 12) && !((__CUDA_ARCH__
// >= 800) || !defined(__CUDA_ARCH__))
//__device__ floatX __ldcs(const floatX* address) {
//   unsigned short bf = __ldcs(reinterpret_cast<const unsigned
//   short*>(address)); return __nv_bfloat16_raw{bf};
// }
//
//__device__ void __stcs(floatX* address, floatX value) {
//   __stcs(reinterpret_cast<unsigned short*>(address),
//   ((__nv_bfloat16_raw)value).x);
// }
// #endif
...
namespace {
// template <typename T>
//__global__ void embedding_forward(T* output, const int* __restrict__ input,
// const T* __restrict__ weight,
//                           std::size_t B, std::size_t input_length,
...
//   output[b * input_length * embedding_dim + t * embedding_dim + c] =
//   weight[ix * embedding_dim + c];
// }

// ----------------------------------------------------------------------------
// Packed128 data structure, which forces the compiler to use 128-bit
// loads/stores in GPUs that support (the LDG.128 and STS.128 instructions) This
// is a bit similar to the use of float4 in the case of 32-bit floats, but
...
// complain.
// If not, you easily get "no viable overload" (for sm52) and "function already
// exists" (sm_80)
...
__global__ void embedding_forward(floatX* out, const int* inp,
                                  const floatX* wte, const floatX* wpe, int B,
                                  int T, int C) {
  // short-form typedefs
...
// template <typename T>
//__global__ void embedding_backward(const T* grad_output, int* grad_input, T*
// grad_weight,
//                                    std::size_t B, std::size_t input_length,
//                                    std::size_t embedding_dim) {
//   auto tid = threadIdx.x + blockIdx.x * blockDim.x;
...
//
//   atomicAdd(&grad_weight[ix * embedding_dim + c], grad_output[b *
//   input_length * embedding_dim + t * embedding_dim + c]);
//
// }
...
// This gives us a random number from threadIdx/blockIdx + a single seed for the
// entire GPU todo - possibly overkill and we don't need such high quality
// random numbers? (tbd) http://eiserloh.net/noise/SquirrelNoise5.hpp
__device__ __host__ constexpr unsigned int SquirrelNoise5(int positionX,
                                                          unsigned int seed) {
  constexpr unsigned int SQ5_BIT_NOISE1 =
      0xd2a80a3f;  // 11010010101010000000101000111111
  constexpr unsigned int SQ5_BIT_NOISE2 =
      0xa884f197;  // 10101000100001001111000110010111
  constexpr unsigned int SQ5_BIT_NOISE3 =
      0x6C736F4B;  // 01101100011100110110111101001011
  constexpr unsigned int SQ5_BIT_NOISE4 =
      0xB79F3ABB;  // 10110111100111110011101010111011
  constexpr unsigned int SQ5_BIT_NOISE5 =
      0x1b56c4f5;  // 00011011010101101100010011110101
...
__device__ __host__ constexpr unsigned int Get2dNoiseUint(int indexX,
                                                          int indexY,
                                                          unsigned int seed) {
  constexpr int PRIME_NUMBER =
      198491317;  // Large prime number with non-boring bits
  return SquirrelNoise5(indexX + (PRIME_NUMBER * indexY), seed);
}

// stochastic rounding built on top of Squirel Noise above (with seed updated
// per step via xorshift)
__device__ __forceinline__ void stochastic_rounding(float in,
                                                    __nv_bfloat16* out,
                                                    unsigned int seed) {
  ...
  float_bits = (rounded_bits > threshold) ? (float_bits | 0xFFFF)
                                          : (float_bits & ~0xFFFF);
  *out = __float2bfloat16_rn(__uint_as_float(float_bits));
}
__device__ __forceinline__ void stochastic_rounding(float in, half* out,
                                                    unsigned int random) {
...
__device__ void atomicStochasticAdd(T* address, float val0, float val1,
  ...
  do {
    assumed = old;
    float2 new_fp32 =
        make_float2((float)(reinterpret_cast<T*>(&old)[0]) + val.x,
...
__device__ void atomicStochasticAdd(float* address, float val0, float val1,
                                    unsigned int seed) {
...
__global__ void embedding_backward(floatX* dwte, floatX* dwpe,
                                   const floatX* dout, const int* inp, int B,
                                   int T, int C, unsigned int seed) {
...
void forwardKernel(cudaStream_t cudaStream, Tensor3D& output,
  ...
  auto f = [&](auto dummy) {
    ...
    embedding_forward<<<grid_size, block_size, 0, cudaStream>>>(
        static_cast<floatX*>(output.data()),
        static_cast<const int*>(input.data()),
        static_cast<const floatX*>(wte.data()),
        static_cast<const floatX*>(wpe.data()), B, L, embedding_dim);
  };
...
void backwardKernel(cudaStream_t cudaStream, const Tensor3D& grad_output,
  ...
  auto f = [&](auto dummy) {
    ...
    embedding_backward<<<grid, block, 0, cudaStream>>>(
        static_cast<floatX*>(grad_wte.data()),
        static_cast<floatX*>(grad_wpe.data()),
        static_cast<const floatX*>(grad_output.data()),
        static_cast<int*>(grad_input.data()), B, L, embedding_dim,
        random_u32(&rng_state));
  };
