...
namespace multihead_attn::fused_softmax::scaled_masked_softmax {
...
void fwd_cuda(const dllm::ContextCompute* context,
  ...
  const int batches = input.size<0>();
  const int pad_batches = mask.size<1>();
  const int attn_heads = input.size<1>();
  const int query_seq_len = input.size<2>();
  const int key_seq_len = input.size<3>();
  ...
  const void* mask_ptr = mask.data_ptr();
...
void bwd_cuda(const dllm::ContextCompute* context,
  ...
  auto& softmax_results = softmax_results_;
  ...
  // torch::Tensor input_grads = torch::empty(
  //     {batches, attn_heads, query_seq_len, key_seq_len}, act_options);
