...
void set_params(Masked_multihead_attention_params<T> &params,
                ...
                const int nnz_heads, T *q_ptr, T *k_ptr, T *v_ptr,
                T *k_cache_ptr, T *v_cache_ptr, int *length_per_sample,
                T *rotary_cos, T *rotary_sin, T *out_ptr, int *nnz_head_idx) {
...
void single_query_attention(
    ...
    const torch::Tensor &v, torch::Tensor &k_cache, torch::Tensor &v_cache,
    ...
    c10::optional<const torch::Tensor> nnz_head_idx_, const int timestep,
  ...
  CHECK_SHAPE(k_cache, batch_size, nheads_kv, headdim / packsize,
      ...
      q.scalar_type(), "single_query_attention", [&] {
        ...
        set_params(
            ...
            length_per_sample_.has_value()
                ? length_per_sample_.value().data_ptr<int>()
                : nullptr,
            rotary_cos_.has_value()
                ? reinterpret_cast<DataType *>(rotary_cos_.value().data_ptr())
                : nullptr,
            rotary_sin_.has_value()
                ? reinterpret_cast<DataType *>(rotary_sin_.value().data_ptr())
                : nullptr,
            reinterpret_cast<DataType *>(out.data_ptr()),
            nnz_head_idx_.has_value() ? nnz_head_idx_.value().data_ptr<int>()
                                      : nullptr);
