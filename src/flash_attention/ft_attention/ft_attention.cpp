...
template <typename T>
void cross_multihead_attention(
    const Masked_multihead_attention_params<T> &params,
    const cudaStream_t &stream);
...
void set_params(Masked_multihead_attention_params<T> &params,
                const size_t batch_size, const size_t nheads,
                const size_t nheads_kv, const size_t memory_max_seqlen,
                const size_t headdim, const int timestep,
                const int rotary_embedding_dim, const float rotary_base,
                const bool neox_rotary_style, const int q_batch_stride,
                const int k_batch_stride, const int v_batch_stride,
                const int nnz_heads, const T *q_ptr, const T *k_ptr,
                const T *v_ptr, T *k_cache_ptr, T *v_cache_ptr,
                const int *length_per_sample, const T *rotary_cos,
                const T *rotary_sin, T *out_ptr, const int *nnz_head_idx) {
...
void single_query_attention(
    const dllm::ContextCompute *context, torch::Tensor<3> &out,
    const torch::Tensor<3> &q, const torch::Tensor<3> &k,
    const torch::Tensor<3> &v, torch::Tensor<5> &k_cache,
    torch::Tensor<4> &v_cache,
    c10::optional<const torch::Tensor<1>> length_per_sample_,
    c10::optional<const torch::Tensor<2>> rotary_cos_,
    c10::optional<const torch::Tensor<2>> rotary_sin_,
    c10::optional<const torch::Tensor<1>> nnz_head_idx_, const int timestep,
    int rotary_embedding_dim = 0, const float rotary_base = 10000.0f,
  ...
  auto batch_size = v_cache.size<0>();
  auto nheads = q.size<1>();
  auto nheads_kv = v_cache.size<1>();
  auto memory_max_seqlen = v_cache.size<2>();
  auto headdim = v_cache.size<3>();
  ...
  CHECK_SHAPE(q, IntArrayRef3D{batch_size, nheads, headdim});
  CHECK_SHAPE(k, IntArrayRef3D{batch_size, nheads_kv, headdim});
  CHECK_SHAPE(v, IntArrayRef3D{batch_size, nheads_kv, headdim});
  CHECK_SHAPE(v_cache,
              IntArrayRef4D{batch_size, nheads_kv, memory_max_seqlen, headdim});
  // k_cache shape: [B, H, Dh/x, L, x] where x=8 for fp16 and x=4 for fp32
  int packsize = k_cache.dtype == torch::kFloat32 ? 4 : 8;
  CHECK_SHAPE(k_cache, IntArrayRef5D{batch_size, nheads_kv, headdim / packsize,
                                     memory_max_seqlen, packsize});
  ...
  if (length_per_sample_.has_value()) {
    ...
    CHECK_SHAPE(length_per_sample, IntArrayRef1D{batch_size});
  ...
  if (nnz_head_idx_.has_value()) {
    ...
    int nnz_heads = nnz_head_idx.size<0>();
    CHECK_SHAPE(nnz_head_idx, IntArrayRef1D{nnz_heads});
  ...
  DISPATCH_FLOAT_AND_HALF_AND_BF16(
      q.scalar_type(), "single_query_attention", [&] {
        using DataType = typename SATypeConverter<scalar_t>::Type;
        Masked_multihead_attention_params<DataType> params;
        set_params<DataType>(
            params, batch_size, nheads, nheads_kv, memory_max_seqlen, headdim,
            timestep, rotary_embedding_dim, rotary_base, neox_rotary_style,
            //            q.stride(0), k.stride(0), v.stride(0),
            // nnz_head_idx_.has_value() ? nnz_head_idx_.value().size(0) : 0,
            q.stride<0>(), k.stride<0>(), v.stride<0>(),
            nnz_head_idx_.has_value() ? nnz_head_idx_.value().size<0>() : 0,
            reinterpret_cast<const DataType *>(q.data_ptr()),
            reinterpret_cast<const DataType *>(k.data_ptr()),
            reinterpret_cast<const DataType *>(v.data_ptr()),
            reinterpret_cast<DataType *>(k_cache.data_ptr()),
            reinterpret_cast<DataType *>(v_cache.data_ptr()),
            length_per_sample_.has_value()
                ? length_per_sample_.value().data_ptr<const int>()
                : nullptr,
            rotary_cos_.has_value() ? reinterpret_cast<const DataType *>(
                                          rotary_cos_.value().data_ptr())
                                    : nullptr,
            rotary_sin_.has_value() ? reinterpret_cast<const DataType *>(
                                          rotary_sin_.value().data_ptr())
                                    : nullptr,
            reinterpret_cast<DataType *>(out.data_ptr()),
            nnz_head_idx_.has_value()
                ? nnz_head_idx_.value().data_ptr<const int>()
                : nullptr);
