...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      // device pointers
                      const at::Tensor<3> q, const at::Tensor<3> k,
                      const at::Tensor<3> v, at::Tensor<3> out,
...
void set_params_dgrad(Flash_bwd_params &params,
                      ...
                      // device pointers
                      const at::Tensor<3> &q, const at::Tensor<3> &k,
                      const at::Tensor<3> &v, const at::Tensor<3> &out,
                      at::Tensor<3> dout, at::Tensor<3> dq, at::Tensor<3> dk,
                      at::Tensor<3> dv, void *cu_seqlens_q_d,
  ...
  auto out = [&]() {
    if (out_.has_value()) {
      ...
      TORCH_CHECK(out->stride<-1>() == 1,
                  "Output tensor must have contiguous last dimension");
      CHECK_SHAPE(*out,
                  IntArrayRef4D{batch_size, sizes[1], sizes[2], head_size_og});
      if (seqlenq_ngroups_swapped) {
...
auto mha_varlen_fwd(
    ...
    c10::optional<at::Tensor<1>> &alibi_slopes_,  // num_heads or b x num_heads
  ...
  if (!paged_KV) {
    ...
    CHECK_SHAPE(v, IntArrayRef4D{num_blocks, page_block_size, num_heads_k,
                                 head_size_og});
    CHECK_SHAPE(block_table, IntArrayRef2D{batch_size, max_num_blocks_per_seq});
  }
  ...
  if (seqused_k.has_value()) {
    ...
    CHECK_SHAPE(seqused_k_, IntArrayRef1D{batch_size});
  }
  ...
  //                                 opts.dtype(at::kFloat));
  auto softmax_lse = torch::empty<dllm::CUDA>(
      IntArrayRef3D{batch_size, num_heads, max_seqlen_q}, at::kFloat, context);
  // at::Tensor p;
  // Only return softmax if there's dropout to reduce compilation time
  // if (return_softmax) {
  //   TORCH_CHECK(p_dropout > 0.0f,
  ...
  if (seqlenq_ngroups_swapped) {
    ...
    set_params_splitkv(context, params, batch_size, num_heads, head_size,
                       max_seqlen_k, max_seqlen_q, head_size_rounded, p_dropout,
                       /*num_splits*/ 0, dprops);
  }
...
auto mha_bwd(
    ...
    const at::Tensor<4> &k,  // batch_size x seqlen_k x num_heads_k x head_size
    const at::Tensor<4> &v,  // batch_size x seqlen_k x num_heads_k x head_size
    const at::Tensor<4> &out,  // batch_size x seqlen_q x num_heads x head_size
    const at::Tensor<3> &softmax_lse,  // b x h x seqlen_q
    c10::optional<std::shared_ptr<at::Tensor<4>>>
  ...
  //   dimension"); CHECK_SHAPE(dq, batch_size, seqlen_q, num_heads, head_size);
  // } else {
  ...
  auto [dq, dk, dv] = [&]() {
    ...
    if (dk_.has_value()) {
      ...
      CHECK_SHAPE(*dk,
                  IntArrayRef4D{batch_size, seqlen_k, num_heads_k, head_size});
    } else {
...
auto mha_varlen_bwd(
    ...
    c10::optional<at::Tensor<1>> &rng_state) {
#ifdef FLASHATTENTION_DISABLE_BACKWARD
  TORCH_CHECK(false, "This flash attention build does not support backward.");
#endif
  ...
  //   // num_heads, head_size_rounded) because that would be too large if there
  //   is
  //   // a very long sequence and the rest of the sequences are short. Instead,
  //   we
  ...
  //   we're
  //   // normally allowed to do. So we won't have to do any bound checking, and
  ...
  auto dq_accum = [&]() {
    if (loop) {
      ...
      // num_heads, head_size_rounded). Note that 128 is the max block size on
      // the seqlen_q dimension. For dQ, the i-th sequence is stored in indices
      // from cu_seqlens[i] + 128 * i to cu_seqlens[i + 1] * 128 * i - 1. This
      // ensures that the i-th sequence and (i + 1)-th sequence will be at least
      // 128 apart. It's ok for us to do atomicAdds up to 128 rows beyond what
  ...
  set_params_dgrad(
      ...
      k, v, out, dout_padded, dq, dk_expanded, dv_expanded,
      cu_seqlens_q.data_ptr(), cu_seqlens_k.data_ptr(),
      loop ? dq_accum.data_ptr() : nullptr, nullptr, nullptr,
      softmax_lse.data_ptr(), softmax_d->data_ptr(), p_dropout, softmax_scale,
      window_size_left, window_size_right, deterministic);
  params.dq_accum_split_stride = !deterministic ? 0 : dq_accum.stride(0);
...
auto mha_fwd_kvcache(
  ...
  if (!paged_KV) {
    ...
    CHECK_SHAPE(kcache, IntArrayRef4D{num_blocks, page_block_size, num_heads_k,
                                      head_size_og});
    CHECK_SHAPE(vcache, IntArrayRef4D{num_blocks, page_block_size, num_heads_k,
                                      head_size_og});
